...
class _InsInProcessInstallRunner(object):
	...
	def run(self, source, dest_dir):
		...
		# If source and dest are same file, do nothing.
		try:
			if os.path.samefile(source, dest):
				return True
		except Exception:
			# Simply ignore the error. Following code should handle
			# this case, or capture its error.
			pass

		# To emulate install command, remove the dest file in advance.
		try:
			os.unlink(dest)
		except OSError as e:
