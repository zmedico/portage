...
class _multiprocessing_proxy(proxy.objectproxy.ObjectProxy):
    ...
    it will only affect portage internals, which ensures that it does
    not other multiprocessing usage within the process, such as for the
    pytest-xdist plugin (see bug 924416).
    """
    ...
    def set_start_method(self, method, force=False):
        if method == "spawn":
            # Enable subprocesses to inherit this setting.
            os.environ["PORTAGE_MULTIPROCESSING_START_METHOD"] = "spawn"
        else:
            os.environ.pop("PORTAGE_MULTIPROCESSING_START_METHOD", None)
        global _multiprocessing_context
