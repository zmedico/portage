...
class RepoConfig:
    ...
    def __init__(self, name, repo_opts, local_config=True):
        ...
        self.sync_rcu = repo_opts.get("sync-rcu", "false").lower() in ("true", "yes")
    ...
    def get_pregenerated_cache(self, auxdbkeys, readonly=True, force=False):
        ...
        """
        return next(
            self.iter_pregenerated_caches(auxdbkeys, readonly=readonly, force=force),
            None,
        )
...
class RepoConfigLoader:
    ...
    def _add_repositories(
        ...
        if overlays:
            ...
            for ov in overlays:
                ...
                if isdir_raise_eaccess(ov) or (base_priority == 0 and ov is portdir):
    ...
    def __init__(self, paths, settings):
        ...
        portdir = self._add_repositories(
            portdir,
            portdir_overlay,
            prepos,
            ignored_map,
...
def parse_layout_conf(repo_location, repo_name=None):
    ...
    if manifest_hashes is not None:
        ...
        missing_required_hashes = manifest_required_hashes.difference(manifest_hashes)
