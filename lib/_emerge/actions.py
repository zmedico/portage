...
def action_regen(settings, portdb, max_jobs, max_load):
    ...
    regen = MetadataRegen(portdb, max_jobs=max_jobs, max_load=max_load, main=True)
...
def adjust_config(myopts, settings):
    ...
    if fail_clean is not None:
        if fail_clean is True and "fail-clean" not in settings.features:
            settings.features.add("fail-clean")
        elif fail_clean == "n" and "fail-clean" in settings.features:
...
def validate_ebuild_environment(trees):
    ...
    for myroot in trees:
        ...
        settings.validate()
        features_warn.update(_emerge_features_warn.intersection(settings.features))
...
def expand_set_arguments(myfiles, myaction, root_config):
    ...
    for a in myfiles:
        if a.startswith(SETPREFIX):
            ...
            if s == "installed":
                msg = (
                    "The @installed set is not recommended when "
